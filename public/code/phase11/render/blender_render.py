"""Phase 11 — Physically accurate Blender Cycles renderer.

Rendering improvements:
- AgX tone mapping (Filmic fallback for 3.x)
- Transmissive water shader with Fresnel-correct reflections
- Foam vertex colors -> white diffuse shader mix
- Shadow caustics + path guiding (when available)
- Proper light path bounces for water
- Reduced smooth iterations (3 vs 8) to preserve surface detail
- Adaptive sampling

Usage:
    blender --background --python blender_render.py -- \
        --meshes ./export/flood/meshes --frame 0,30,60,90 --output ./renders
"""
import bpy
import math
import os
...
def setup_render():
    ...
    if BL_IS_3X:
        ...
        try:
            ...
            sc.view_settings.view_transform = 'Filmic'
            sc.view_settings.look = 'Medium High Contrast'
            print("  Tone mapping: Filmic (AgX not available)")

    # --- Light path bounces ---
    # Opaque muddy water doesn't need deep transmission chains
    ...
    # --- Shadow caustics (Blender 3.2+) ---
    if hasattr(sc.cycles, 'caustics_refractive'):
        sc.cycles.caustics_refractive = True
        sc.cycles.caustics_reflective = False
        print("  Shadow caustics: enabled (refractive)")

    # --- Path guiding (Blender 3.4+) ---
    ...
    if hasattr(sc.cycles, 'use_adaptive_sampling'):
        ...
        print("  Adaptive sampling: threshold=0.01")
    ...
    # Sun at ~40 degrees elevation, matching the sun lamp direction
    sky.sun_direction = (0.50, -0.30, 0.64)  # normalized, elevation ~40°
    sky.turbidity = 3.0  # clearer sky = brighter reflections on water
    L.new(sky.outputs["Color"], bg.inputs["Color"])
    L.new(bg.outputs["Background"], out.inputs["Surface"])


# ---------------------------------------------------------------------------
# Building import — OBJ with MTL textures
# ---------------------------------------------------------------------------
def import_building(meta):
    ...
    print(f"  Building: {len(meshes)} meshes, {len(verts):,} verts (native coords)")
    print(f"  Bounds: {bmin.round(2)} -> {bmax.round(2)}")
    return objects, bmin, bmax, bcenter


# ---------------------------------------------------------------------------
# Water material — physically correct transmissive shader with foam
# ---------------------------------------------------------------------------
def mat_water(edge_bounds=None):
    """Muddy flood water — opaque brown with surface reflection + foam.

    Real flood water is NOT transparent glass. You can't see more than
    10-20cm through it. This uses a mostly-opaque brown surface with
    Fresnel reflection (IOR 1.33) and strong bump for waves.
    """
    ...
    # --- Water BSDF: opaque muddy brown with Fresnel reflection ---
    bsdf = N.new("ShaderNodeBsdfPrincipled"); bsdf.location = (300, 200)
    bsdf.inputs["Base Color"].default_value = (0.10, 0.07, 0.03, 1)  # dark mud
    bsdf.inputs["Roughness"].default_value = 0.12  # slightly glossy surface
    bsdf.inputs["IOR"].default_value = 1.333
    bsdf.inputs["Metallic"].default_value = 0.0
    # Mild transmission for translucency at thin edges only
    if BL_IS_3X:
    ...
    foam_bsdf.inputs["Metallic"].default_value = 0.0

    # --- Mix water + foam based on vertex color ---
    ...
    # Foam factor from vertex color attribute "foam"
    ...
    # --- Multi-scale bump (strong — visible waves) ---
    ...
    # Combine: 60% swell + 30% chop + 10% ripple
    if BL_IS_3X:
    ...
    # --- Volume Absorption: depth-dependent darkening ---
    vol_abs = N.new("ShaderNodeVolumeAbsorption"); vol_abs.location = (800, -300)
    vol_abs.inputs["Color"].default_value = (0.12, 0.18, 0.08, 1)  # greenish-brown tint
...
def mat_flood_base():
    """Flood base plane — same opaque muddy look as main water.

    Must visually match the water surface so mesh gaps are invisible.
    Slightly darker to suggest depth.
    """
    ...
    # Match water: opaque muddy brown, slightly darker (suggests depth)
    ...
    # Wave bump matching the water surface
...
def create_ground(center_xy, z=0.0, size=10.0):
    ...
    bsdf.inputs["Base Color"].default_value = (0.08, 0.06, 0.04, 1)  # wet muddy ground
    bsdf.inputs["Roughness"].default_value = 0.65  # wet pavement is glossier
...
def setup_lights(center, size):
    ...
    # Sun direction aligned with sky node: (0.50, -0.30, 0.64)
    # Elevation ~40°, azimuth ~-30° from +X
    sun.rotation_euler = (math.radians(50), math.radians(10), math.radians(-30))

    # Enable shadow caustics on sun lamp
...
# ---------------------------------------------------------------------------
# Water mesh loading with foam vertex colors
# ---------------------------------------------------------------------------
water_obj = None
water_mat = None
_inv_scale = 1.0
...
def _apply_foam_vertex_colors(mesh, foam_data):
    """Apply foam values as a vertex color layer named 'foam'."""
    if foam_data is None or len(foam_data) == 0:
        return

    n_verts = len(mesh.vertices)
    if len(foam_data) != n_verts:
        print(f"    Foam mismatch: {len(foam_data)} vs {n_verts} verts, skipping")
        return
    ...
    # Set values: foam -> RGBA (foam, foam, foam, 1.0)
    n = len(attr.data)
...
def _apply_foam_vertex_colors_3x(mesh, foam_data):
    """Apply foam as vertex color layer for Blender 3.x."""
    if foam_data is None or len(foam_data) == 0:
        return
...
def load_water_mesh(obj_path, foam_path=None):
    ...
    # Transform sim [0,1] -> native coords
    ...
    if water_obj is not None:
        ...
        if foam_data is not None:
            if BL_IS_3X:
                _apply_foam_vertex_colors_3x(new_mesh, foam_data)
            else:
        ...
        if foam_data is not None:
            if BL_IS_3X:
                _apply_foam_vertex_colors_3x(mesh, foam_data)
            else:
        ...
        # Reduced smoothing (3 iterations vs 8) to preserve surface detail
...
def render_frames():
    ...
    print("=" * 60)
    print("Phase 11 — Physically Accurate Flood Renderer")
